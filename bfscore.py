...
def evaluate_bf_score(segmentation_masks, gt, match_threshold: int = 16):
    scores = []
    pred_fg = segmentation_masks
    print("pred fg mask shape")
    print(pred_fg.shape)
    print("gt shape")
    print(gt.shape)
    for k in range(gt.size(0)):
        ...
        if len(np.unique(pred_mask)) == 1:
            ...
            score, areas_gt = bfscore(gt_fg_mask, pred_mask, threshold=match_threshold, verbose=True)
